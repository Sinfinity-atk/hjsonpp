...
public class ShieldCoreBlock extends AdvancedCoreBlock {
    ...
    public ShieldCoreBlock(String name){
        super(name);
        update = true;
        solid = true;
    ...
    protected static final Cons<Bullet> shieldConsumer = bullet -> {
        if(bullet.team != paramEntity.team && bullet.type.absorbable && !bullet.absorbed && Intersector.isInRegularPolygon(((ShieldCoreBlock)(paramEntity.block)).sides, paramEntity.x, paramEntity.y, paramEntity.realRadius(), ((ShieldCoreBlock)(paramEntity.block)).shieldRotation, bullet.x, bullet.y)){
            bullet.absorb();
    ...
    public void setBars(){
        super.setBars();
        addBar("shield", (ShieldCoreBlockBuild entity) -> new Bar("stat.shieldhealth", Pal.accent, () -> entity.broken ? 0f : 1f - entity.buildup / (shieldHealth + phaseShieldBoost * entity.phaseHeat)).blink(Color.white));
    }
    ...
    public class ShieldCoreBlockBuild extends AdvancedCoreBuild{
        ...
        public void overwrote(Seq<Building> previous){
            if(previous.size > 0 && previous.first().block == block && previous.first() instanceof ShieldCoreBlockBuild b){
                broken = b.broken;
                buildup = b.buildup;
            }
        }
